...
def procesar_respuesta_valida(user_input):
    pregunta_seguimiento = cadena_reaccion.run(noticia=st.session_state.noticia_actual, reaccion=user_input).strip() # Agregado noticia_actual
    if st.session_state.contador_preguntas == 0:
        ...
        st.session_state.conversacion_para_guardar.append({"pregunta": pregunta_seguimiento, "respuesta": ""}) # Guardar pregunta de seguimiento
        st.session_state.pregunta_pendiente = True
        st.session_state.contador_preguntas += 1
    else:
        st.session_state.conversacion_para_guardar[-1]["respuesta"] = user_input # Actualizar respuesta a la pregunta pendiente
...
if "historial" not in st.session_state:
    ...
    st.session_state.reacciones = [] # Esta lista se usa para el LLM, no para guardar todo.
    st.session_state.conversacion_para_guardar = [] # Nueva lista para guardar todas las Q&A
    st.session_state.mostrada_noticia = False
    st.session_state.contador_preguntas = 0
    ...
    st.session_state.noticia_actual = "" # Para almacenar la noticia actual
...
if st.session_state.pregunta_general_idx < len(preguntas_inversor):
    ...
    if not any(p["contenido"] == pregunta_actual for p in st.session_state.historial if p["tipo"] == "bot"):
        ...
        st.session_state.conversacion_para_guardar.append({"pregunta": pregunta_actual, "respuesta": ""}) # Guardar la pregunta inicial

    user_input = st.chat_input("Escribe tu respuesta aquí...")
    if user_input:
        st.session_state.historial.append({"tipo": "user", "contenido": user_input})
        st.session_state.conversacion_para_guardar[-1]["respuesta"] = user_input # Actualizar la respuesta
        st.session_state.reacciones.append(user_input) # Mantener para el análisis LLM
...
# Noticias ESG
    ...
    if not st.session_state.mostrada_noticia:
        ...
        st.session_state.noticia_actual = noticia # Guardar la noticia actual
        ...
        st.session_state.conversacion_para_guardar.append({"pregunta": texto_noticia, "respuesta": ""}) # Guardar la pregunta de la noticia
        st.session_state.mostrada_noticia = True

    user_input = st.chat_input("Escribe tu respuesta aquí...")
    if user_input:
        st.session_state.historial.append({"tipo": "user", "contenido": user_input})
        st.session_state.conversacion_para_guardar[-1]["respuesta"] = user_input # Actualizar la respuesta de la noticia

        if st.session_state.pregunta_pendiente:
            # Esta parte se ejecuta cuando el usuario responde a una pregunta de ampliación
            # La respuesta ya se añadió arriba a conversacion_para_guardar[-1]["respuesta"]
            st.session_state.reacciones.append(user_input) # Mantener para el análisis LLM
            st.session_state.contador += 1
            st.session_state.mostrada_noticia = False
            st.session_state.contador_preguntas = 0
            ...
            # Esta parte se ejecuta cuando el usuario da su primera respuesta a una noticia
            evaluacion = cadena_evaluacion.run(respuesta=user_input).strip().lower()
            if evaluacion == "false":
                # Si la respuesta es "False", se pide una ampliación.
                # La respuesta del usuario ya se guardó, ahora guardamos la pregunta de ampliación.
                pregunta_ampliacion = cadena_reaccion.run(noticia=st.session_state.noticia_actual, reaccion=user_input).strip() # Agregado noticia_actual
                with st.chat_message("bot", avatar="🤖"):
                    st.write(pregunta_ampliacion)
                st.session_state.historial.append({"tipo": "bot", "contenido": pregunta_ampliacion})
                st.session_state.conversacion_para_guardar.append({"pregunta": pregunta_ampliacion, "respuesta": ""}) # Guardar la pregunta de ampliación
                st.session_state.pregunta_pendiente = True
            else:
                # Si la respuesta es "True", la procesamos como válida y avanzamos.
                st.session_state.reacciones.append(user_input) # Mantener para el análisis LLM
                st.session_state.contador += 1
                st.session_state.mostrada_noticia = False
                st.session_state.contador_preguntas = 0
        ...
        with st.form("formulario_final"):
            ...
            if enviar:
                try:
                    ...
                    # Preparar los datos de la conversación para la fila
                    chat_data = []
                    for item in st.session_state.conversacion_para_guardar:
                        chat_data.append(f"Q: {item['pregunta']}")
                        chat_data.append(f"A: {item['respuesta']}")
